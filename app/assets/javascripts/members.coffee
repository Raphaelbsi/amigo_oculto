  ...
  $('body').on 'click', 'a.remove_member', (e) ->
    $.ajax '/members/'+ e.currentTarget.id,
        ...
        success: (data, text, jqXHR) ->
           M.toast({html: 'Membro removido', 4000, classes: 'green'})
          $('#member_' + e.currentTarget.id).remove()
        error: (jqXHR, textStatus, errorThrown) ->
          M.toast({html: 'Problema na remoção de membro', 4000, classes: 'red'})
    return false
  ...
  $('.new_member').on 'submit', (e) ->
    $.ajax e.target.action,
        ...
        success: (data, text, jqXHR) ->
          ...
          $('#member_name').focus()
          M.toast({html: 'Membro adicionado', 4000, classes: 'green'})
        error: (jqXHR, textStatus, errorThrown) ->
          M.toast({html: 'Problema na hora de incluir membro', 4000, classes: 'red'})
    return false
